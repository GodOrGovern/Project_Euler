...
ring and middle ring score double and treble scores respectively. At the centre
of the board are two concentric circles called the bull region, or bulls-eye.
The outer bull is worth 25 points and the inner bull is a double, worth 50
points. There are many variations of rules but in the most popular game the
players will begin with a score 301 or 501 and the first player to reduce their
...
their current score it is called a "checkout" and the highest checkout is 170:
T20 T20 D25 (two treble 20s and double bull). Note that D1 D2 is considered
...
shall not include misses in considering combinations; for example, D3 is
the same as 0 D3 and 0 0 D3. How many distinct ways can a player checkout
...
def main():
    ...
    for n in range(1, 21):
        ...
        doubles += [2*n]
    total = 0
    for i, n1 in enumerate(all_spots):
        for n2 in all_spots[i:]:
            for n3 in doubles:
                if n1+n2+n3 < 100:
                    total += 1
    print(total)
